...
public sealed class DesktopBridge
{
    ...
    private async void OnWebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        try
        {
            ...
            if (request is null || string.IsNullOrWhiteSpace(request.Id)) return;
    ...
    private static BridgeResult SelectDirectory(JsonElement payload, Window owner)
    {
        ...
        var dialog = new OpenFolderDialog { Title = "选择下载目录", Multiselect = false };
        if (!string.IsNullOrWhiteSpace(initialDirectory) && Directory.Exists(initialDirectory))
            dialog.InitialDirectory = initialDirectory;

        var ownerHandle = new WindowInteropHelper(owner).Handle;
        var result = ownerHandle == IntPtr.Zero ? dialog.ShowDialog() : dialog.ShowDialog(owner);
    ...
    private static BridgeResult OpenDirectory(JsonElement payload)
    {
        ...
        if (string.IsNullOrWhiteSpace(path)) return BridgeResult.Fail("路径为空");
    ...
    private sealed class BridgeResult
    {
        ...
        public static BridgeResult Ok(object data)    => new() { Success = true,  Data = data };
        public static BridgeResult Fail(string error) => new() { Success = false, Error = error };
    }
}
